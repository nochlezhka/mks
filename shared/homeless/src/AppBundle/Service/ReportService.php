                             ...
                             $createServicedateFrom = null, $createServiceFromTo = null, $homelessReason = null, $disease = null, $breadwinner = null, $branchId = null)
    {
        ...
        switch ($type) {
            ...
            case static::DELIVERY:
                $result = $this->delivery($dateFrom, $dateTo, $userId, $branchId);
                break;
        }
...
//        }
        ...
        if ($createServicedateFrom || $createServiceFromTo) {
            ...
            JOIN `fos_user_user` AS u ON u.`id` = s.`created_by_id`
            JOIN `branches` AS b ON b.`id` = u.`branch_id`
            WHERE c.`created_at` >= :createServicedateFrom AND c.`created_at` <= :createServiceFromTo '. ($branchId ? 'AND u.`branch_id` = :branchId' : '') . '
            ORDER BY st.`sort`'
            );
...
//        }
    ...
    private function delivery($dateFrom = null, $dateTo = null, $userId = null, $branchId = null)
    {
        $fields = [
            'категория',
...
FROM AppBundle\Entity\Delivery d
  ...
  JOIN d.createdBy cb
  JOIN cb.branch cbb
WHERE d.deliveredAt >= :dateFrom AND d.deliveredAt <= :dateTo ' . ($userId ? 'AND d.createdBy IN (' . implode(',', $userId) . ')' : '') . '  ' . ($branchId ? 'AND cb.branch = :branchId' : '') . '
