...
(when-not (js/localStorage.getItem "branch-id") (js/localStorage.setItem "branch-id" 0))
...
(defn search-page []
  (r/with-let [
      ...
      switch-selected! (fn [item-id]
                         (js/localStorage.setItem "branch-id" item-id)
                         (dispatch [:delivery-items-get item-id]))

      check-and-search! (fn [val branch-id]
                          (if (> branch-id 0)
                            (dispatch [:client-search val])
                            (dispatch [:show-notification :danger "Перед началом поиска необходимо выбрать город выдачи"])))]
    ...
    [:section.section>div.container>div.content
     [:section.columns
      ...
      (let [clients @(subscribe [:search])
            ...
            not-found (= clients [])
            branch-items @(subscribe [:branch-items])]
        [:div.column
         [:h3 "Выберите город выдачи"]
         [:div {:class ["select" (when (:branch-items loading) "is-loading")]}
          [:select {:value  (js/localStorage.getItem "branch-id") :on-change #(switch-selected! (-> % .-target .-value))}
           [:option
            {:disabled true :value 0}]
           (doall (for [{id :id name :name} branch-items]
                    ^{:key id} [:option { :value id } (str name)]))]]
         ...
         [:div {:class ["control" (when (:search loading) "is-loading")]}
          [:input
           {:class ["input" "is-large" (when not-found "is-danger")]
            :type "text"
            :placeholder "ФИО..."
            :on-change #(check-and-search! (-> % .-target .-value) (js/localStorage.getItem "branch-id"))
            }]
         ...
         [:button.button {:on-click #(dispatch [:push-state :search])} "Завершить выдачу"]]
          ...
          [:div.column.is-4
           ...
           [:p>a {:href (apply href (delivery-params :crutches))}
            [:button.button.is-large.is-block "3. Аксессуары"]]
           [:p>a {:href (apply href (delivery-params :rehabilitation))}
            [:button.button.is-large.is-block "4. Костыли/трости"]]
           [:p>a {:href (apply href (delivery-params :dishes))}
            [:button.button.is-large.is-block "5. Посуда"]]
           [:p>a {:href (apply href (delivery-params :gift))}
            [:button.button.is-large.is-block "6. Подарок"]]
           ...
           [kb/kb-action "4" #(dispatch (into [:push-state] (delivery-params :rehabilitation)))]
           [kb/kb-action "5" #(dispatch (into [:push-state] (delivery-params :dishes)))]
           [kb/kb-action "6" #(dispatch (into [:push-state] (delivery-params :gift)))]
           ]
...
(defn delivery-page []
  (r/with-let [
               ...
               heading (str "Выдача " (kind {:clothes "одежды"
                                             ...
                                             :crutches "аксессуаров"
                                             :rehabilitation "костылей и тростей"
                                             :dishes "посуды"
                                             :gift "подарков"
                                             }))
